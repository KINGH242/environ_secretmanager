...
@pytest.fixture
def esm():
    with patch('environ_secretmanager.env_secrets.secretmanager.SecretManagerServiceClient') as mock_client:
...
def test_get_env_secret_from_gsm(monkeypatch, esm):
    ...
    with patch.object(esm, 'access_secret_version', return_value='gsm_value') as mock_access:
